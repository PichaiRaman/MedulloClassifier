...
getGenesUpTop <- function(x, pvalcutoff=0.05, top=200)
{
	tmpTable <- topTable(output[[1]], x, 20000)
...
getGenesDownTop <- function(x, pvalcutoff=0.05, top=200)
{
	tmpTable<- topTable(output[[1]], x, 20000)
	tmpTable <- tmpTable[tmpTable[,"logFC"]<0,]
	tmpTable <- tmpTable[order(tmpTable[,"adj.P.Val"]),][1:top,]
...
WNTGeneCombosUp <- Reduce(intersect, list(getGenesComboUp(4, 150),getGenesComboUp(5, 150),getGenesComboUp(6, 150)))
...
###########################
g4GeneCombosUpvsG3<- getGenesComboUp(9, 100)
g3GeneCombosUpvsG4<- getGenesComboUp(12, 100)
...
#Let's optimize this to find best solutions mostly for G3 and G4
getScoreCombo <- function(geneInds=rep(1, length(g3GeneCombosUp)+length(g4GeneCombosUp)))
{
	#Get Genes
	print(geneInds);
	tmpMarker <- length(g3GeneCombosUp);
	g3GeneCombosUpTmp <- g3GeneCombosUp[(as.logical(geneInds)[1:tmpMarker])]
	tmpMarker2 <- tmpMarker+length(g4GeneCombosUp)
	g4GeneCombosUpTmp <- g4GeneCombosUp[(as.logical(geneInds)[(tmpMarker+1):tmpMarker2])]

	medulloGeneSetsUpTmp <- list("WNT"=WNTGeneCombosUp, "SHH"=shhGeneCombosUp, "Group3"=g3GeneCombosUpTmp, "Group4"=g4GeneCombosUpTmp)
	myMat <- calcScoreMat(geneRatioOut, medulloGeneSetsUpTmp);
	myClass <- colnames(myMat)[max.col(myMat,ties.method="first")]
	myScore <- (1-sum(myClass==as.character(sampAnnot[,2]))/97)
	print(myScore)
	myScore <- ifelse(is.na(myScore), 1, myScore)
	return(myScore)
}

monitor <- function(obj) {

print(paste("Min Value is ", min(obj$evaluations)))
hist(obj$evaluations);
}

rbga.results = rbga.bin(size=(length(g3GeneCombosUp)+length(g4GeneCombosUp))
	,monitorFunc=monitor,evalFunc=getScoreCombo, verbose=TRUE, mutationChance=0.05, iters=20, popSize=100)

#Confirm best score
bestScore <- min(rbga.results$best)
iterBestScore <- rbga.results$evaluations==bestScore

for(i in 1:sum(iterBestScore))
{
	geneInds <- rbga.results$population[iterBestScore,][i,]
	tmpMarker <- length(g3GeneCombosUp);
	g3GeneCombosUpTmp <- g3GeneCombosUp[(as.logical(geneInds)[1:tmpMarker])]
	tmpMarker2 <- tmpMarker+length(g4GeneCombosUp)
	g4GeneCombosUpTmp <- g4GeneCombosUp[(as.logical(geneInds)[(tmpMarker+1):tmpMarker2])]
	...
	myClass <- colnames(myMat)[max.col(myMat,ties.method="first")]
	myScore <- sum(myClass==as.character(sampAnnot[,2]))/97
	saveRDS(medulloGeneSetsUpTmp, "../../results/V5_CombosUp/medulloSetsUp.RDS")
	class109401 <- classifyGSE109401();
	class37418 <- classifyGSE37418();
	print(paste("iteration", i, "WO filtering Accuracy is", round(class109401,4), "and", round(class37418,4)));
